...
const App: React.FC = () => {
  ...
  const handleConfirmAnswer = () => {
    ...
    if (currentQuestionIndex +1 < TOTAL_QUESTIONS) stopMusic('suspense'); 
        ...
        if (currentQuestionIndex === TOTAL_QUESTIONS - 1) {
          ...
          stopAllMusic(); 
           ...
           setTimeout(() => {
            ...
            setDisabledOptions([]);
            setGameState(GameState.QUESTION_ASKING);
            if (currentQuestionIndex +1 < TOTAL_QUESTIONS) playMusic('suspense'); 
          }, 3000); 
        }
      } else {
        playSound('wrong');
  ...
  const handleUseAskAudience = () => {
    ...
    const votes: AudienceVote[] = currentQuestion.options.map(option => {
      let percentage = Math.floor(Math.random() * 30) + 10; 
    ...
    if(currentSum !== 100 && normalizedVotes.length > 0) {
        ...
        normalizedVotes.sort((a,b) => b.percentage - a.percentage); 
          ...
          {currentQuestion.options.map((option, index) => (
            <button
              ...
              disabled={
                ...
                (gameState === GameState.ANSWER_SELECTED && selectedAnswer !== option && selectedAnswer !== null) 
              }
